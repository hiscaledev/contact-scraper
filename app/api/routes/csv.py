...
from app.core.database import get_all_jobs, get_job_status
from app.schemas.csv import CSVUploadResponse, JobError, JobStatus
from app.services.csv_service import start_csv_processing
from app.services.linkedin_csv_service import start_linkedin_csv_processing
...
async def upload_csv(
    ...
    website_column: str = Form(
        default="website", description="Name of the column containing website URLs"
    ),
...
@router.post(
    ...
    response_model=CSVUploadResponse,
    tags=["CSV Processing"],
    summary="Upload CSV file for batch LinkedIn-only scraping",
    description="Upload a CSV file with website URLs to scrape LinkedIn URLs only (fast, no AI)",
)
async def upload_linkedin_csv(
    ...
    """
    Upload a CSV file for batch LinkedIn-only processing.

    This endpoint:
    - Scrapes only LinkedIn URLs from homepage
    - No AI validation or contact page detection
    - Much faster than full contact scraping (2-5 seconds per row)
    - Processing happens in the background with max 2 concurrent jobs
    - Files are stored in Supabase storage
    ...
    - raw_json_response (full JSON response)
    ...
    try:
        ...
        # Start background processing with worker pool (returns integer job ID)
        job_id = start_linkedin_csv_processing(content, file.filename, website_column)
...
async def get_job(job_id: int, api_key: str = Depends(verify_api_key)):
    """
...
async def download_csv(job_id: int, api_key: str = Depends(verify_api_key)):
    """
    ...
    if not output_path:
        raise HTTPException(
            status_code=404, detail="Output file path not found in job data"
        )
...
async def get_jobs_list(
    status: Optional[str] = Query(
        None, description="Filter by status (queued, processing, completed, failed)"
    ),
    limit: int = Query(
        50, description="Maximum number of jobs to return", ge=1, le=100
    ),
    api_key: str = Depends(verify_api_key),
):
    """
